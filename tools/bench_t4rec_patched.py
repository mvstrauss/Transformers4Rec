#!/usr/bin/env python3
"""
Transformers4Rec Production Benchmark (v3)
==========================================

Extends bench_t4rec_v2.py with PyTorch Profiler integration for
kernel-level trace collection (compatible with TraceLens analysis).

New in v3:
  - torch.profiler integration (--profile) producing Chrome trace JSON
  - Per-rank trace files for distributed profiling (--profile-ranks)
  - Configurable profiler schedule (--profile-wait/warmup/active/repeat)
  - TraceLens-compatible output for ops breakdown and NCCL analysis
...
Usage:
    # Data directory defaults (adjust to your system):
    #   Home storage (persists): /home/$USER/t4rec_data/prod_50m
    #   Local NVMe (ephemeral):  /nvme_local/t4rec_data/prod_50m
    # For profiling runs, home storage is fine (dataloader is not the
    # bottleneck in short runs).  For throughput benchmarks, copy to
    # NVMe first if network I/O jitter matters.

    DATA=/home/$USER/t4rec_data/prod_50m

    # Basic training benchmark:
    python bench_t4rec_v3.py --data-dir $DATA \
        --model-size xlarge --max-seq-len 200 --bf16

    # 8-GPU DDP with breakdown + eval:
    torchrun --nproc_per_node=8 bench_t4rec_v3.py \
        --data-dir $DATA \
        --model-size xlarge --max-seq-len 200 --bf16 --ddp \
        --streaming --breakdown --max-steps-per-epoch 500 --epochs 1 \
        --output-json results_xlarge_bf16_8gpu.json

    # Single-GPU profiling (quick, 30 steps):
    python bench_t4rec_v3.py \
        --data-dir $DATA \
        --model-size xlarge --max-seq-len 200 --bf16 --streaming \
        --max-steps-per-epoch 30 --epochs 1 \
        --profile --profile-dir ./traces/xlarge_1gpu

    # 8-GPU DDP profiling (rank 0 and 4, with tensor shapes):
    torchrun --nproc_per_node=8 bench_t4rec_v3.py \
        --data-dir $DATA \
        --model-size xlarge --max-seq-len 200 --bf16 --ddp --streaming \
        --max-steps-per-epoch 50 --epochs 1 \
        --profile --profile-dir ./traces/xlarge_8gpu \
        --profile-ranks 0,4 --profile-record-shapes

    # 8-GPU DDP profiling with call stacks (larger traces):
    torchrun --nproc_per_node=8 bench_t4rec_v3.py \
        --data-dir $DATA \
        --model-size xlarge --max-seq-len 200 --bf16 --ddp --streaming \
        --max-steps-per-epoch 50 --epochs 1 \
        --profile --profile-dir ./traces/xlarge_8gpu_stacks \
        --profile-ranks all --profile-with-stack
    ...
    # Compare results:
    python bench_t4rec_v3.py --compare results_a.json results_b.json

Profiling output:
    Trace files are Chrome trace JSON, viewable in https://ui.perfetto.dev/
    and analyzable with TraceLens:
        TraceLens_generate_perf_report_pytorch <trace.json>
...
parser = argparse.ArgumentParser(
    description="T4Rec production benchmark (v3) — with profiler",
...
parser.add_argument("--skip-dl-bench", action="store_true",
                    help="Skip Benchmark A (dataloader throughput) to save time")
parser.add_argument("--force-rocm-dl", action="store_true",
                    help="Force ROCm/PyArrow streaming dataloader on all platforms "
                         "(bypasses Merlin, eliminates HtoD memcpy per batch)")
...
parser.add_argument("--profile", action="store_true",
                    help="Enable torch.profiler trace collection during Benchmark C")
parser.add_argument("--profile-dir", type=str, default="./traces",
                    help="Output directory for trace files (default: ./traces)")
parser.add_argument("--profile-wait", type=int, default=10,
                    help="Profiler schedule: steps to skip before profiling (default: 10)")
parser.add_argument("--profile-warmup", type=int, default=5,
                    help="Profiler schedule: warmup steps (captured but discarded, default: 5)")
parser.add_argument("--profile-active", type=int, default=3,
                    help="Profiler schedule: steps to actively record traces (default: 3)")
parser.add_argument("--profile-repeat", type=int, default=1,
                    help="Profiler schedule: number of (wait+warmup+active) cycles (default: 1)")
parser.add_argument("--profile-ranks", type=str, default="0",
                    help="Comma-separated ranks to profile, or 'all' (default: '0')")
parser.add_argument("--profile-record-shapes", action="store_true",
                    help="Record tensor shapes in traces (moderate size increase)")
parser.add_argument("--profile-with-stack", action="store_true",
                    help="Record Python call stacks in traces (large size increase)")
...
# ═════════════════════════════════════════════════════════════════════════
# FIX: XLNet positional encoding on GPU
#
# HuggingFace transformers 4.30.2 XLNetModel.relative_positional_encoding
# uses torch.arange() without device=, forcing all sin/cos/einsum/cat ops
# onto CPU and then copying 200 MB to GPU every forward pass (~314 ms on
# H200). This monkey-patch moves the computation to GPU (~0.4 ms).
# ═════════════════════════════════════════════════════════════════════════

def _patch_xlnet_positional_encoding():
    import transformers.models.xlnet.modeling_xlnet as _xlnet

    @staticmethod
    def _fast_pos_emb(pos_seq, inv_freq, bsz=None):
    ...
    def _fast_rpe(self, qlen, klen, bsz=None):
        device = DEVICE
    ...
    _xlnet.XLNetModel.positional_embedding = _fast_pos_emb
    _xlnet.XLNetModel.relative_positional_encoding = _fast_rpe

_patch_xlnet_positional_encoding()
log("Applied XLNet positional encoding GPU fix (torch.arange device= patch)")
...
def detect_platform():
    ...
    if ARGS.force_rocm_dl or is_rocm_available():
        info["dataloader_engine"] = "rocm"
    else:
        info["dataloader_engine"] = "merlin"
    return info
...
def build_dataloader(data_path, schema, batch_size, max_seq_len, shuffle=True,
                     ...
                     streaming=None, rows_per_chunk=None, force_rocm=False):
    from transformers4rec.torch.utils.data_utils import T4RecDataLoader
    from transformers4rec.utils.dependencies import is_rocm_available

    if force_rocm or is_rocm_available():
        engine = "rocm"
    else:
...
class _NullProfiler:
    """No-op stand-in when profiling is disabled or this rank is not profiled."""
    def __enter__(self):
        return self
...
def make_profiler_ctx(args, rank, world_size):
    """Build a ``torch.profiler.profile`` context or a no-op replacement.

    The profiler schedule (wait -> warmup -> active) controls which training
    steps are recorded.  Each time an *active* window completes, the
    ``trace_handler`` exports a Chrome-trace JSON file that TraceLens and
    Perfetto can ingest.

    In DDP runs, only the ranks listed in ``--profile-ranks`` actually
    record traces; all other ranks get a lightweight ``_NullProfiler`` so
    there is zero overhead on those processes.
    """
    if not args.profile:
        return _NullProfiler()

    if not _HAS_PROFILER:
        log("  ** WARNING: torch.profiler not available in this PyTorch build; "
            "skipping profiling")
        return _NullProfiler()

    if args.profile_ranks != "all":
        active_ranks = set()
        for tok in args.profile_ranks.split(","):
            tok = tok.strip()
            if tok:
                active_ranks.add(int(tok))
        if rank not in active_ranks:
            return _NullProfiler()

    trace_dir = os.path.join(args.profile_dir,
                             f"w{world_size}")
    ...
    def trace_handler(p):
        end_step = p.step_num
        start_step = end_step - active_steps + 1
        trace_path = os.path.join(
            trace_dir,
            f"rank{rank}_step{start_step}_{end_step}.json",
        )
...
def _profile_step_budget(args):
    """Minimum training steps required for the profiler schedule to fire."""
    if not args.profile:
        return 0
    return (args.profile_wait + args.profile_warmup + args.profile_active) \
        * args.profile_repeat
...
def compare_results(file_a, file_b):
    ...
    W = 72
    print("=" * W)
    print("TRANSFORMERS4REC BENCHMARK COMPARISON (v3)")
    print("=" * W)
...
def main():
    ...
    log("=" * 72)
    log("Transformers4Rec Production Benchmark (v3)")
    log("=" * 72)
    ...
    if ARGS.profile:
        log(f"  Profiling       : ENABLED")
        log(f"    trace dir     : {ARGS.profile_dir}")
        log(f"    schedule      : wait={ARGS.profile_wait} warmup={ARGS.profile_warmup} "
            f"active={ARGS.profile_active} repeat={ARGS.profile_repeat}")
        log(f"    profiled ranks: {ARGS.profile_ranks}")
        log(f"    record_shapes : {ARGS.profile_record_shapes}")
        log(f"    with_stack    : {ARGS.profile_with_stack}")
        min_steps = _profile_step_budget(ARGS)
        log(f"    min steps req : {min_steps} (for schedule to complete)")
    ...
    if ARGS.data_dir:
        ...
        tmpdir = os.path.join(tempfile.gettempdir(), "t4rec_bench_v3_data")
    ...
    if ARGS.mode == "inference" or ARGS.skip_dl_bench:
        log("\n  [Skipping Benchmark A (dataloader throughput)]")
    else:
        log("\n" + "=" * 72)
        ...
        if ARGS.breakdown:
            ...
            if IS_DDP:
                log(f"  Note: DDP all-reduce is overlapped with backward. "
                    f"Use --profile for kernel-level decomposition.")
    ...
    # ── 7. BENCHMARK C — Full epoch (with optional profiling) ────────────
    if ARGS.mode in ("train", "both"):
        log("\n" + "=" * 72)
        log("BENCHMARK C: Full training epochs (end-to-end)")
        if ARGS.profile:
            log("             + torch.profiler trace collection")
        log("=" * 72)
        ...
        # Warn if profiler schedule won't complete within available steps
        if ARGS.profile:
            total_train_steps = ARGS.warmup_steps + ARGS.epochs * max_steps_per_epoch
            min_prof_steps = _profile_step_budget(ARGS)
            if total_train_steps < min_prof_steps:
                log(f"\n  ** PROFILER WARNING: schedule needs {min_prof_steps} "
                    f"steps but Benchmark C will only run {total_train_steps} "
                    f"(warmup={ARGS.warmup_steps} + {ARGS.epochs}x"
                    f"{max_steps_per_epoch}).")
                log(f"     Increase --max-steps-per-epoch or --epochs, or "
                    f"reduce --profile-wait/warmup/active.")
        ...
        prof = make_profiler_ctx(ARGS, LOCAL_RANK, WORLD_SIZE)
        ...
        # -- Collect trace file list (after profiler context exits) ----
        if ARGS.profile:
            trace_subdir = os.path.join(ARGS.profile_dir, f"w{WORLD_SIZE}")
            if os.path.isdir(trace_subdir):
                trace_files_written = sorted(
                    f for f in os.listdir(trace_subdir) if f.endswith(".json")
                )
        ...
        # -- Profiling metadata ----------------------------------------
        if ARGS.profile and trace_files_written:
            abs_trace_dir = os.path.abspath(
                os.path.join(ARGS.profile_dir, f"w{WORLD_SIZE}"))
            results["profiling"] = {
                "enabled": True,
                "trace_dir": abs_trace_dir,
                "trace_files": trace_files_written,
                "schedule": {
                    "wait": ARGS.profile_wait,
                    "warmup": ARGS.profile_warmup,
                    "active": ARGS.profile_active,
                ...
                "profiled_ranks": ARGS.profile_ranks,
                "record_shapes": ARGS.profile_record_shapes,
                "with_stack": ARGS.profile_with_stack,
            }
            log(f"\n  Profiler traces : {len(trace_files_written)} files in "
                f"{abs_trace_dir}")
            for tf in trace_files_written:
                full = os.path.join(abs_trace_dir, tf)
    ...
    if "profiling" in results:
        ...
        log(f"  Profiler traces      : {len(pr['trace_files'])} files")
        log(f"  Trace dir            : {pr['trace_dir']}")
        log(f"  View traces          : https://ui.perfetto.dev/")
        log(f"  TraceLens report     : TraceLens_generate_perf_report_pytorch <trace.json>")
